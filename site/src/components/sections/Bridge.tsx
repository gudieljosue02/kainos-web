...
import { Waveform, Cpu, Monitor, ArrowRight, ArrowDown } from "@phosphor-icons/react/dist/ssr";
        ...
        <Reveal delay={200}>
          <div className="mt-5 shell md:mt-20">
            <div className="shell-inner overflow-hidden rounded-[inherit]">
              {/* 3-panel grid: raw | processor | output */}
              <div className="grid grid-cols-1 md:grid-cols-[1fr_160px_1fr] lg:grid-cols-[1fr_180px_1fr]">
                <RawInputPanel />
                <ProcessorStrip />
                <OutputPanel />
              </div>
...
/* ── Panel 1: Raw legacy input ────────────────────────────── */
function RawInputPanel() {
  return (
    <div className="relative flex flex-col border-b border-hairline/60 bg-[#04080f] md:border-b-0 md:border-r md:border-hairline/60">
      {/* Header strip */}
      <div className="flex items-center gap-2 border-b border-hairline/40 px-3 py-2.5 md:px-5 md:py-3">
        ...
        <div>
          <div className="font-mono text-[8.5px] uppercase tracking-[0.22em] text-white/35">
            01 · Source
          </div>
          <div className="text-[11px] font-medium leading-tight text-white/60">
            Legacy Ultrasound
          </div>
        </div>
      ...
      {/* CRT display */}
      <div className="relative min-h-[200px] flex-1 p-3 md:min-h-[300px] md:p-5">
        <CrtDisplay />
      </div>

      {/* Footer */}
      <div className="border-t border-hairline/40 px-3 py-2 md:px-5">
...
/* ── Center: Alethia Kit processor strip ─────────────────── */
function ProcessorStrip() {
  return (
    <div className="relative flex flex-col items-center justify-between border-b border-hairline/60 bg-gradient-to-b from-[#061326] via-[#071a38] to-[#061326] py-3 md:border-b-0 md:border-r md:border-hairline/60 md:py-0">
      {/* Mobile: horizontal layout */}
      <div className="flex w-full items-center justify-between px-4 md:hidden">
        <ArrowDown size={14} className="text-[#4fc3f7]/60 rotate-0" />
        <div className="flex flex-col items-center gap-1">
          <span className="flex h-8 w-8 items-center justify-center rounded-full border border-[#4fc3f7]/30 bg-[#4fc3f7]/10 text-[#4fc3f7]">
            <Cpu size={14} weight="light" />
          </span>
          <span className="font-mono text-[8px] uppercase tracking-[0.2em] text-[#4fc3f7]/80">
            Alethia Kit
          </span>
        </div>
        <ArrowDown size={14} className="text-[#4fc3f7]/60" />
      </div>

      {/* Desktop: vertical layout */}
      <div className="hidden w-full flex-1 flex-col items-center justify-between px-3 py-5 md:flex">
        {/* Top arrow */}
        <div className="flex flex-col items-center gap-1">
        ...
        <div className="flex w-full flex-col items-center gap-3">
          ...
          <div className="text-center">
            ...
            <div className="mt-0.5 text-[11px] font-semibold tracking-tight text-[#4fc3f7]/90">
              Alethia Kit
            </div>
          </div>
          ...
          <div className="w-full space-y-1.5 px-1">
            {["INGEST", "INFER", "EMIT"].map((label, i) => (
              <div key={label} className="flex items-center gap-1.5">
                <span className="w-[32px] text-right font-mono text-[7px] uppercase tracking-[0.15em] text-white/30">
                  {label}
                </span>
                <div className="relative h-1 flex-1 overflow-hidden rounded-full bg-white/5">
                  <span
                    ...
                    style={{
                      animation: `pipe-flow 2s cubic-bezier(0.4,0,0.6,1) infinite`,
          ...
          <div className="grid w-full grid-cols-4 gap-1 px-1">
            {Array.from({ length: 8 }).map((_, i) => (
              <span
                key={i}
                className="h-2 rounded-[2px] bg-gradient-to-b from-[#4fc3f7]/30 to-[#4fc3f7]/5"
                style={{
...
function OutputPanel() {
  return (
    <div className="relative flex flex-col bg-[#020509]">
      ...
      <div className="relative min-h-[200px] flex-1 p-3 md:min-h-[300px] md:p-5">
        <UltrasoundVisual className="h-full min-h-[176px] rounded-xl md:min-h-[272px]" withScan withAnnotations />
      </div>
...
function CrtDisplay() {
  ...
  return (
    <div
      ref={ref}
      className="relative h-full w-full overflow-hidden rounded-xl border border-white/5 bg-[#060b12]"
    >
      {/* Scanlines overlay */}
      ...
      {/* Raw ultrasound SVG (no annotations) */}
      <svg
        viewBox="0 0 400 300"
      ...
      >
        ...
        <g clipPath="url(#crt-cone)">
          ...
          {/* Heavy noise — makes it look analog/raw */}
          <rect width="400" height="300" filter="url(#crt-noise)" opacity="0.55" />
          {/* Tissue blobs — no clean labels */}
          ...
          <g opacity="0.25">
            {[60, 120, 180, 240].map((y) => (
              <g key={y}>
                ...
                <text x="370" y={y + 3} fontSize="7" fill="rgba(255,255,255,0.5)" fontFamily="monospace">
                  {y / 30}cm
                </text>
              </g>
            ))}
